...
def reading_snap(_path,_snap,_parttype):
    ...
    return ids_,x_,y_,z_,mass_,rho_,vx_,vy_,vz_,vrms_,hsml_,inte_

def sorting_by_ids(_id,_x,_y,_z,_mass,_rho,_vx,_vy,_vz,_vrms,_hsml,_inte):
    id_,x_,y_,z_,mass_,rho_,vx_,vy_,vz_,vrms_,hsml_,inte_ = zip(*sorted(zip(_id,_x,_y,_z,_mass,_rho,_vx,_vy,_vz,_vrms,_hsml,_inte)))
    return np.array(id_),np.array(x_),np.array(y_),np.array(z_),np.array(mass_),np.array(rho_),np.array(vx_),np.array(vy_),np.array(vz_),np.array(vrms_),np.array(hsml_),np.array(inte_)
